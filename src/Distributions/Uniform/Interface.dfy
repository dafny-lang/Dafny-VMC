...
module Uniform.Interface {
  ...
  trait {:termination false} Trait extends Coin.Interface.Trait, UniformPowerOfTwo.Interface.Trait {
    ...
    method UniformIntervalSample(a: int, b: int) returns (u: int)
      ...
      requires a < b
      ensures a <= u < b
      ensures Model.IntervalSample(a, b)(old(s)) == Monad.Terminating(u, s)
    {
      var v := UniformSample(b - a);
      assert Model.Sample(b-a)(old(s)) == Monad.Terminating(v, s);
      assert Model.IntervalSample(a, b)(old(s)) == Monad.Terminating(a + v, s);
      u := a + v;
    }
