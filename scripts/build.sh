...
if [ "$TARGET_LANG" = "py" ]
then
  $DAFNY build --target:$TARGET_LANG src/interop/$TARGET_LANG/Full/DafnyVMC.$TARGET_LANG src/interop/$TARGET_LANG/Part/DafnyVMCPartMaterial.$TARGET_LANG -o build/$TARGET_LANG/DafnyVMC dfyconfig.toml --no-verify
else
